...
Kubernetes Posture Agent — **fourth Phase-1b agent**; **ninth under [ADR-007](../../../docs/_meta/decisions/ADR-007-cloud-posture-as-reference-agent.md)** (F.3 / D.1 / D.2 / D.3 / F.6 / D.7 / D.4 / multi-cloud-posture / **k8s-posture**). **Closes the Phase-1b detection track.**

> **Agent-ID note (2026-05-20).** Prior README copy self-claimed the ID "D.6"; that identifier is reserved for the operator's Compliance agent per the 2026-05-20 remaining-agents sketch §0. This package is referenced by its package name (`k8s-posture`) throughout the codebase going forward.
...
> **Package `__version__` 0.1.0 → 0.2.0 — operator Cycle 8 (2026-06-11).** ADR-010 version-extension bump of the actual `__version__` (the dated maturity narration above predates and is independent of the package-version field). This cycle adds, **alongside** the offline readers (Q1 coexist) and keeping the OCSF 2003 wire shape byte-identical (WI-K5): **live kube-bench + Polaris execution** against running clusters (`tools/kube_bench_live.py` + `tools/polaris_live.py`, byte-identical via the shared parsers) with a **CIS Kubernetes Benchmark v1.8** catalog and Polaris **custom-policy** overlay; a **kubelet API client** (`tools/kubelet_client.py`) feeding **runtime posture rules** (privileged / hostNetwork / run-as-root / dangerous-caps / …) and **basic RBAC analysis** (Q4 heuristic — wildcard roles / broad secret access / cluster-admin→ServiceAccount; **NOT** a full effective-permissions sim, which is v0.3) → OCSF 2003 (new additive `RUNTIME` + `RBAC` finding types, live-path only); **cloud-agnostic auth** (EKS/AKS/GKE/self-managed via kubeconfig, Q2). Single gated lane `NEXUS_LIVE_K8S_POSTURE` (WI-K4 e2e). **Safety:** a scan targets exactly one cluster — `assert_single_cluster_context` + `ClusterScanSession` reject cross-cluster context leak (Q3/WI-K8); kubeconfig secrets are never logged (`redact_kubeconfig`, WI-K9). Setup: [`runbooks/`](runbooks/); per-tool coverage (no aggregate, WI-K1) + the closure record under `docs/_meta/d-6-k8s-posture-v0-2-*`.
>
> **Honest scope (WI-K3):** the live scanners are built + end-to-end tested **through emission** (OCSF 2003); wiring them into the agent's continuous `run()` loop is **v0.3** — the offline `run()` stays the deterministic emitting path (WI-K5). Admission webhooks (Q6), multi-cluster batch (Q3), full effective-permissions RBAC (Q4), and runtime drift are all v0.3+.

## What it does

k8s-posture emits OCSF v1.3 Compliance Findings (`class_uid 2003`) from a Kubernetes cluster's posture, via three concurrent feeds. Given an `ExecutionContract` requesting a Kubernetes posture scan, k8s-posture runs a **six-stage pipeline**:
...
- **Workloads** — k8s-posture's bundled **10-rule analyser** runs over every pod template. Three reader sources, mutually exclusive per run:
...
Three deterministic normalizers (`normalize_kube_bench` + `normalize_polaris` + `normalize_manifest`) lift the typed reader outputs into OCSF v1.3 Compliance Findings via **F.3's re-exported `build_finding`** — k8s-posture emits the **identical wire shape** (`class_uid 2003`) as F.3 cloud-posture and multi-cloud-posture. Downstream consumers (Meta-Harness, D.7 Investigation, fabric routing) already filter on `class_uid 2003`; k8s-posture is invisible at the schema level. The `finding_info.types[0]` discriminator carries `cspm_k8s_cis` / `cspm_k8s_polaris` / `cspm_k8s_manifest`.

A dedicated **DEDUP stage** (new vs multi-cloud-posture) collapses overlapping findings on `(rule_id, namespace, workload_arn, 5min_bucket)`. Higher severity wins with first-seen tiebreak; collapsed loser finding-IDs are preserved on the survivor as a `dedup-sources` evidence entry. Cross-tool collisions (kube-bench vs Polaris/manifest) naturally avoid each other because their arn schemes differ (`k8s://cis/…` vs `k8s://workload/…` vs `k8s://manifest/…`).

Operators see per-namespace breakdown (CIS + Polaris + Manifest counts per namespace) and CRITICAL findings **pinned above** the per-severity sections (mirrors F.3 + D.3 + D.4 + multi-cloud-posture patterns).

## ADR-007 conformance

k8s-posture is the **ninth** agent under the reference template. Inherits v1.1 (LLM adapter via `charter.llm_adapter`; no per-agent `llm.py`) and v1.2 (NLAH loader is a 21-LOC shim over `charter.nlah_loader` — k8s-posture is the **6th native v1.2 agent**). **Not** in the v1.3 always-on class — k8s-posture honours every budget axis. **Does not consume** the v1.4 candidate (sub-agent spawning primitive); single-driver per the agent spec.

**Schema reuse (Q1).** k8s-posture re-exports F.3's `class_uid 2003 Compliance Finding` schema verbatim — `Severity`, `AffectedResource`, `CloudPostureFinding`, `build_finding`, `FindingsReport`, and `FINDING_ID_RE` (`CSPM-<CLOUD>-<SVC>-<NNN>-<context>`). The cloud token is `KUBERNETES` (the regex constrains the cloud segment to `[A-Z]+`, so `K8S` would fail the gate). Adds package-specific `K8sFindingType` enum (3 discriminators) and three per-source severity helpers on top.

LLM use: **not load-bearing** (matches multi-cloud-posture). Normalizers, dedup, and summarizer are all deterministic. The `LLMProvider` parameter on `agent.run` is plumbed but never called in v0.1 — keeps the contract surface stable when Phase 1c adds optional LLM narrative.
...
uv run eval-framework run \
    ...
    --output /tmp/k8s-posture-eval-out.json
...
## The 10-rule manifest analyser (k8s-posture v0.1)
...
| File            | Format                                | Purpose                                                                                                                                       |
| --------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `findings.json` | `FindingsReport.model_dump_json()`    | Wire shape consumed by D.7 Investigation, fabric routing, Meta-Harness. **OCSF 2003 — identical to F.3 cloud-posture + multi-cloud-posture.** |
