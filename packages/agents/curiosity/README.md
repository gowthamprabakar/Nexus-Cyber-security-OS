...
> **v0.2 (Cycle 15 of 17 — the GENERATIVE counterpart to D.7).** 20 tasks, 330+ tests. Adds: **OCSF 2004 emission** alongside the unchanged CuriosityClaim (Q1 — D.12 is the **6th OCSF 2004 emitter**; the claims.> envelope stays byte-identical, WI-X6); **source scope 4 → 14** agents (Q2); **DeepSeek → Anthropic** provider fallback via `charter.llm` (Q3, wrapper under `curiosity/providers/`); **three gap kinds** — region (tunable) + technique + time (Q4); **continuous-monitoring infrastructure** (scheduler + heartbeat/continuous coexistence, NOT run()-wired — Phase C); and **six code-level invariants** — 3 inherited from the D.13 LLM-agent template (`assert_categorical_only`, `assert_bounded_retry`, `assert_coverage_gap_cited`) + 3 NEW curiosity-specific (`assert_tenant_scoped` H5, `assert_no_claims_subscription` producer-only, `assert_llm_only_with_gaps` H4 — see [`runbooks/`](runbooks/)). Deviation profile preserved (WI-X12): empty `build_registry()`, LLM via `charter.llm` only. Substrate seal **empty** all 20 tasks. **Honest scope (WI-X3):** v0.2 is breadth — emission + multi-source + the invariant set — and the 3 gap detectors ship as infrastructure; the region detector remains the run-loop driver, with technique/time wiring + production-loop wiring deferred to Phase C/v0.3.
