"""Hermes Phase 1 (C-2 PR2) — D.13 synthesis skill-candidate proposal.
...
def _write_llm_audit(path: Path, *, n_llm: int) -> None:
    lines = [
        json.dumps(
            {
                "action": "synthesis.llm.call_completed",
...
async def test_proposes_candidate_on_five_llm_stages(tmp_path: Path) -> None:
    ...
    assert kwargs["external_id"].startswith("synthesis:")
    assert kwargs["properties"]["agent_id"] == "synthesis"
