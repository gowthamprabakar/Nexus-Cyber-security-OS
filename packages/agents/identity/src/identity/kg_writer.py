...
class KnowledgeGraphWriter(KnowledgeGraphWriterBase):
    ...
    async def record_sa_credential_ownership(self, grants: Sequence[tuple[str, str]]) -> None:
        """Write GCP service-account credential ownership (slice #3 GCP owner side).

        Each grant is ``(service_account, fingerprint)`` where ``fingerprint`` is
        ``secret_fingerprint(private_key_id)`` — the SAME hash appsec computes from a leaked SA key,
        so leak ⇄ owner converge on one SECRET node with nothing readable stored. Writes IDENTITY(sa)
        --OWNS--> SECRET + the reverse SECRET --OWNED_BY--> IDENTITY the blast-radius walk needs.
        """
        for service_account, fingerprint in grants:
            sa_node = await self.upsert_node(NodeCategory.IDENTITY, service_account, {})
            cred_node = await self.upsert_node(
