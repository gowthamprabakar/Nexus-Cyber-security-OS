...
> **v0.2 (Cycle 16 of 17 — the FINAL v0.2 cycle; SAFETY-CRITICAL by design).** A.1 is the only agent that mutates customer infrastructure. v0.2 adds: **7 action classes** (the 5 v0.1 K8s patches + `privileged-container` disable + `auto-mount-sa-token` disable; host-network/pid/ipc deferred to v0.3); **10 code-level safety invariants** under `invariants/` — the 6 decision heuristics H1-H6 formalized (`default_recommend`, `action_allowlist`, `dry_run_first`, `rollback_mandatory`, `blast_radius`, `idempotent_scoped`) + 2 NEW action-specific (`privileged_authz` WI-A16, `auto_mount_validation` WI-A17) + `tool_proxy` (WI-A14) + `tenant_scoped` (WI-A18); **K8s-relevant source consumption** (k8s-posture + F.3 + D.5, `tools/source_mapping.py`); **batched multi-finding mode** (opt-in, atomic dry-run-first + rollback-on-partial-failure, capped under H5's 50); and **continuous-monitoring infrastructure** (scheduler + heartbeat/continuous coexistence — continuous NEVER auto-escalates the tier, H1 preserved; NOT run()-wired — Phase C). A.1 remains the **sole OCSF 2007 emitter**. Substrate seal **empty** all 22 tasks. The **final v0.2 infrastructure cycle** — the Phase C production-loop wiring sprint launches next. **Honest scope (WI-A3):** v0.2 is breadth (action classes + the safety-invariant set + batched mode); the cloud-K8s source overlap is thin (most F.3/D.5 findings are cluster/control-plane, not workload patches), and host-\* actions + cloud-native (S3/IAM) remediation + canary/scheduled tiers are v0.3.
...
Operator surface: the `remediation promotion` CLI subcommand group (`status` / `init` / `advance` / `demote` / `reconcile`) — and, as of **v0.1.2**, the `--promotion <path>` flag on `remediation run` itself, routing the loaded `PromotionTracker` through the pre-flight stage gate. v0.1.x is fully backwards-compatible: omitting `--promotion` on `run` preserves v0.1 behaviour exactly. Full migration guide for v0.1 operators in [the runbook §14](runbooks/remediation_workflow.md#14-v01--v0-1-migration); schema reference in [§13](runbooks/remediation_workflow.md#13-promotionyaml-schema-reference-v011). The v0.1.1 verification record (gates, coverage delta, ADR-007 conformance, four-boundary process notes, the permanent `reconcile_matches` limitation) is at [`a1-v0-1-1-verification-2026-05-17.md`](../../../docs/_meta/a1-v0-1-1-verification-2026-05-17.md). The v0.1.2 companion record (the CLI wiring as the first worked example of ADR-010's small-PR scaling) is at [`a1-v0-1-2-verification-2026-05-17.md`](../../../docs/_meta/a1-v0-1-2-verification-2026-05-17.md).
