"""Continuous remediation scheduler (remediation v0.2 Task 19, Q6/WI-A2).

Per **WI-A2** this is continuous-monitoring **INFRASTRUCTURE only**: it decides WHEN each tenant is
due for a re-run, deterministically (caller-provided ``now``), with independent per-tenant
intervals. It does **not** run a remediation and is **not** wired into ``agent.run()`` — driving
the production loop is the **Phase C** consolidated retrofit (the discipline every v0.2 cycle has
held). Tenant-scoped throughout (WI-A18). Pure + deterministic.
"""
...
class RemediationScheduler:
    """Tracks per-tenant re-run intervals + computes which tenants are due."""
    ...
    def register(self, tenant_id: str, *, interval_seconds: int) -> None:
        if not tenant_id:
            raise ValueError("tenant_id must be non-empty (tenant-scoped, WI-A18)")
    ...
    def due(self, now: datetime) -> list[str]:
        """The tenants due for a re-run at ``now`` — never-run ones are always due."""
        out: list[str] = []
