"""Tests for the `threat-intel` CLI (Task 15)."""
...
from threat_intel.cli import main
...
def test_run_with_no_inputs_emits_empty_report_and_warns(tmp_path: Path) -> None:
    ...
    # Warning routed to stderr; click's test runner mixes both into output.
    assert "warning: no" in result.output
    assert (tmp_path / "ws" / "findings.json").is_file()
    assert (tmp_path / "ws" / "report.md").is_file()


def test_run_digest_lists_all_four_finding_types(tmp_path: Path) -> None:
    contract_path = _contract_yaml(tmp_path)
    result = CliRunner().invoke(main, ["run", "--contract", str(contract_path)])
    assert "threat_intel_cve_in_kev_catalog: 0" in result.output
    assert "threat_intel_ioc_match_network: 0" in result.output
    assert "threat_intel_ioc_match_runtime: 0" in result.output
    assert "threat_intel_attack_technique_observed: 0" in result.output


def test_run_digest_lists_severity_counts(tmp_path: Path) -> None:
    contract_path = _contract_yaml(tmp_path)
    result = CliRunner().invoke(main, ["run", "--contract", str(contract_path)])
...
def test_run_writes_attribution_footer_in_report_md(tmp_path: Path) -> None:
    """Q6 CC-BY-4.0 attribution footer must appear even on empty runs."""
    contract_path = _contract_yaml(tmp_path)
    result = CliRunner().invoke(main, ["run", "--contract", str(contract_path)])
    assert result.exit_code == 0
...
def test_run_help_lists_all_six_input_flags() -> None:
    result = CliRunner().invoke(main, ["run", "--help"])
    assert result.exit_code == 0
    assert "--nvd-snapshot" in result.output
    assert "--kev-snapshot" in result.output
    assert "--mitre-attack-snapshot" in result.output
    assert "--vulnerability-workspace" in result.output
    assert "--network-threat-workspace" in result.output
    assert "--runtime-threat-workspace" in result.output
