...
import pytest
from multi_cloud_posture.nlah_loader import default_nlah_dir, load_system_prompt
...
def test_default_nlah_dir_resolves_to_package() -> None:
    ...
    assert d.is_dir()
    assert d.name == "nlah"
    # Must sit inside the multi_cloud_posture package.
    assert "multi_cloud_posture" in str(d).split("/")
...
def test_load_system_prompt_returns_combined() -> None:
    ...
    assert "Multi-Cloud Posture Agent" in prompt
    # tools.md content shows up.
    assert "read_azure_findings" in prompt
    assert "normalize_gcp" in prompt
    # Examples land in the prompt.
    assert "Azure Defender" in prompt
...
def test_loader_is_21_loc_shim() -> None:
    ...
    import multi_cloud_posture.nlah_loader as mod
