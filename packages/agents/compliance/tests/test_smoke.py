...
4. F.3 schema re-export — cloud_posture.schemas reachable (Q1
   substrate; D.6 is the 3rd re-exporter of class_uid 2003 after
   D.5 + D.8 patterns).
5. D.5 finding-shape reference — data_security.schemas reachable
   (D.6 reads D.5 findings.json sibling workspaces per Q3).
...
def test_package_imports() -> None:
    ...
    assert compliance.__version__ == "0.1.0"


def test_charter_llm_adapter_import_works() -> None:
    """ADR-007 v1.1 — D.6 is the **thirteenth** consumer of the hoist."""
...
def test_charter_nlah_loader_import_works() -> None:
    """ADR-007 v1.2 — D.6 is the **ninth** agent shipped natively against v1.2
...
def test_cloud_posture_schema_reexport_available() -> None:
    """D.6 re-exports F.3's ``class_uid 2003 Compliance Finding`` (Q1).
...
def test_data_security_schema_import_works() -> None:
    """D.6 reads D.5 Data Security findings via sibling-workspace
    `findings.json` (Q3). The schema module must import cleanly so
    Task 7's correlator can validate the minimal D.5 fields D.6
    cares about.
    """
...
def test_no_premature_charter_compliance_substrate() -> None:
    """ADR-007 3rd-consumer hoist anti-pattern guard — D.6 v0.1 keeps the
    ...
    requires the 3rd-consumer rule — D.6 is the 1st consumer; revisit
    ...
    assert importlib.util.find_spec("charter.compliance_framework") is None, (
        ...
        "D.6 v0.1 keeps framework loader agent-local per ADR-007 3rd-consumer rule. "
