"""Hermes Phase 1 (C-2 PR3) — D.7 investigation skill-candidate proposal.
...
from investigation.agent import _propose_skill_candidate
...
def _write_llm_audit(path: Path, *, n_llm: int) -> None:
    lines = [
        json.dumps(
            {
                "action": "investigation.llm.call_completed",
...
async def test_proposes_candidate_on_five_llm_stages(tmp_path: Path) -> None:
    ...
    assert kwargs["external_id"].startswith("investigation:")
