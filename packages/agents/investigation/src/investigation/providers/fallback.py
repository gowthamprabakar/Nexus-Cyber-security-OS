"""DeepSeek-primary + Anthropic-fallback LLM provider (investigation v0.2 Task 5, Q3).

Inherits the D.13 (Cycle 13) institutional pattern: a thin ``LLMProvider`` wrapper that tries
the **primary** (DeepSeek via the charter openai-compatible adapter) and, on a fallback-eligible
failure, retries the same call on the **fallback** (Anthropic). Composes **charter providers
only** — no per-agent ``llm.py`` (ADR-007 v1.1; the ``test_no_per_agent_llm_module`` guard), no
direct SDK import. ``should_fallback`` triggers only on transient (5xx / rate-limit / timeout)
failures; ``provider_used`` records which provider answered (WI-I17).
"""
...
def make_resilient_provider(*, primary: LLMProvider, fallback: LLMProvider) -> FallbackLLMProvider:
    ...
    failure (Q3). ``provider_used`` is recorded per call (WI-I17)."""
