"""Continuous investigation scheduler (investigation v0.2 Task 20, Q6/WI-I9).

Per **WI-I9** this is continuous-monitoring **INFRASTRUCTURE only**: it decides WHEN each tenant
is due for a re-investigation, deterministically (caller-provided ``now``), with independent
per-tenant intervals. It does **not** run an investigation and is **not** wired into
``agent.run()`` — driving the production loop is the **Phase C** consolidated retrofit (the same
discipline every v0.2 cycle has held). Tenant-scoped throughout (WI-I16). Pure + deterministic.
"""
...
class InvestigationScheduler:
    """Tracks per-tenant re-investigation intervals + computes which tenants are due."""
    ...
    def register(self, tenant_id: str, *, interval_seconds: int) -> None:
        if not tenant_id:
            raise ValueError("tenant_id must be non-empty (tenant-scoped, WI-I16)")
    ...
    def due(self, now: datetime) -> list[str]:
        """The tenants due for re-investigation at ``now`` — never-run ones are always due."""
        out: list[str] = []
