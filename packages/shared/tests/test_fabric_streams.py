"""Tests for StreamSpec declarations (per ADR-004 + F.7 v0.1 plan Task 2).

The `ADR_004_TABLE` constant below is the ADR-004 "The five buses" table
encoded as test data. Any divergence between the table and the StreamSpec
declarations is a contract violation; updating either side requires
updating the other in the same PR + a doc note in the ADR if the change
is semantic, not just a typo.
"""
...
def test_subject_pattern_matches_adr004(named_stream: tuple[str, StreamSpec]) -> None:
    name, spec = named_stream
    expected = ADR_004_TABLE[name]["subject_pattern"]
    assert spec.subjects == (expected,)


def test_retention_seconds_matches_adr004_days(
    named_stream: tuple[str, StreamSpec],
) -> None:
    name, spec = named_stream
    expected_days = ADR_004_TABLE[name]["retention_days"]
...
def test_all_streams_contains_five_specs() -> None:
    assert len(ALL_STREAMS) == 5


def test_all_streams_declaration_order_mirrors_adr004() -> None:
    assert [s.name for s in ALL_STREAMS] == list(ADR_004_TABLE.keys())
...
def test_no_overlapping_subject_roots() -> None:
    ...
    assert roots == {"events", "findings", "commands", "approvals", "audit"}
