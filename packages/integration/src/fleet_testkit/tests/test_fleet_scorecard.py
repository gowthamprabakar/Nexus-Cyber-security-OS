...
_COVERAGE: list[tuple[str, str]] = [
    ...
    ("k8s RBAC privilege escalation", "full"),
    ("KMS key / encryption exposure", "full"),
...
def test_coverage_denominator_number() -> None:
    ...
    assert (full, partial, total) == (
        ...
        2,
        22,
    )  # +privesc(#13) +leaked_cred(#17) +rbac(#20) +host_vuln(#15) +lateral(#14)
