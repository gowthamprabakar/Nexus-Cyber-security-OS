"""Path-8 (external cross-account trust) capability bank — identity + data-security."""

from pathlib import Path

import pytest

from fleet_testkit.bank_runner import run_external_trust_case
from fleet_testkit.capability import evaluate, load_test_case

_BANK = Path(__file__).parent / "banks" / "path8_external_trust"
...
async def test_case_meets_pass_criteria(case: Path) -> None:
    result = await run_external_trust_case(case)
    evaluate(result, load_test_case(case).pass_criteria)


@pytest.mark.asyncio
async def test_bank_aggregate_precision_recall() -> None:
    assert _CASES, "no path-8 bank cases found"
    tp = fp = fn = 0
    rows = []
    for case in _CASES:
        r = await run_external_trust_case(case)
        tp += r.true_positives
    ...
    print("\n=== Path 8 (external cross-account trust) capability bank ===")
