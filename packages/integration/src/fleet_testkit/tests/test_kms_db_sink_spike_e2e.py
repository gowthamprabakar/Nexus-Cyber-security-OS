...
@pytest.mark.parametrize("kind,key", [("kms-key", "arn:aws:kms:us-east-1:1:key/abc"),
...
async def test_kms_or_db_access_reaches_data_with_no_walker_change(kind, key) -> None:
    async with in_memory_semantic_store() as store:
        ...
        res_id = await store.upsert_entity(tenant_id=_T, entity_type=NodeCategory.CLOUD_RESOURCE.value,
                                           external_id=key, properties={"kind": kind})
        data_id = await store.upsert_entity(tenant_id=_T, entity_type=NodeCategory.DATA_CLASSIFICATION.value,
                                            external_id=f"{key}:data", properties={"data_type": "ssn"})
        await store.add_relationship(tenant_id=_T, src_entity_id=res_id, dst_entity_id=data_id,
