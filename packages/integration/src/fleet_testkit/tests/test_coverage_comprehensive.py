...
async def test_comprehensive_coverage_climb() -> None:
    async with in_memory_semantic_store() as store:
        ident, appsec, cloud, net, k8s, vuln = (
            IdentityKgWriter(store, _T),
            AppsecKgWriter(store, _T),
            CloudKgWriter(store, _T),
            NetKgWriter(store, _T),
            K8sKgWriter(store, _T),
            VulnKgWriter(store, _T),
        )

        # credential access: leaked + stored + kms
        await appsec.record_leaked_credentials("o/a", [_LK])
        await ident.record_credential_ownership([("u:ci", _LK)])
        await ident.record_access([("u:ci", "arn:aws:s3:::lk")])
        await _pub_data(store, "arn:aws:s3:::lk")
        ...
        await cloud.record_stored_secrets(stored_secret_grants([("svc:w", [_SK])]))
        await ident.record_credential_ownership([("u:bot", _SK)])
        await ident.record_access([("u:bot", "arn:aws:s3:::st")])
        await _pub_data(store, "arn:aws:s3:::st")
        await cloud.record_kms_protected_data([("arn:aws:kms:us-east-1:1:key/k", "ssn")])
        await ident.record_access([("r:app", "arn:aws:kms:us-east-1:1:key/k")])
        ...
        await ident.record_access([("r:admin", "arn:aws:s3:::pe")])
        await _pub_data(store, "arn:aws:s3:::pe")
        role = IamRole(
            arn="arn:aws:iam::111111111111:role/p",
            name="p",
            role_id="A",
            create_date=None,
        ...
        await ident.record_external_trust([p for p, _ in xa])
        await ident.record_assume_grants(xa)
        await ident.record_access([("arn:aws:iam::111111111111:role/p", "arn:aws:s3:::xa")])
        await _pub_data(store, "arn:aws:s3:::xa")
        ...
        await k8s.record_inventory(
            ClusterInventory(
                ...
                namespaces=("prod",),
                service_accounts=(
                    K8sServiceAccount(name="ci", namespace="prod", role_arn="r:eks"),
                ),
                roles=(
                    Role(
                        name="admin",
        ...
        await ident.record_access([("r:eks", "arn:aws:s3:::k8")])
        await _pub_data(store, "arn:aws:s3:::k8")
        ...
        await k8s.record_pod_reachability(
            pod_reach_grants(
                (
                    PodRef(f"{cl}/namespace/prod/pod/fh", "prod"),
                    PodRef(f"{cl}/namespace/prod/pod/vic", "prod"),
                )
            )
        )
        vp = await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.K8S_OBJECT.value,
            external_id=f"{cl}/namespace/prod/pod/vic",
            properties={"kind": "pod"},
        )
        vi = await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="vi:1",
            properties={"kind": "container-image"},
        )
        vc = await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CVE_FINDING.value,
            external_id="CVE-POD",
        ...
        await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="arn:vpc-a",
            properties={"is_public": True},
        )
        await net.record_peering_reachability(
            peering_reach_grants(
                (VpcInstance("arn:vpc-a", "vpc-a"), VpcInstance("arn:vpc-b", "vpc-b")),
                frozenset({frozenset({"vpc-a", "vpc-b"})}),
            )
        )
        await _vuln(store, "arn:vpc-b")

        # exposure/vuln: public workload + image vuln (internet_exposed_vulnerable), exposed DB, exposed KMS, SBOM
        pw = await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="arn:web",
            properties={"is_public": True},
        )
        wi = await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="web:1",
            properties={"kind": "container-image"},
        ...
        await store.upsert_entity(
            ...
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="arn:db",
            properties={"kind": "rds-instance", "is_public": True},
        )
        await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="arn:kms2",
            properties={"kind": "kms-key", "is_public": True},
        )
        await _pub_data(
            store, "arn:aws:s3:::secretbucket", dt="private_key"
        )  # public_secret (secret data-type)
        ...
        assert report.coverage_pct >= 50.0, (
            "comprehensive fixture must measure well above the 29% partial baseline"
        )
