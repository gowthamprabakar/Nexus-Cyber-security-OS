...
async def _data_on(store, resource_key):
    d = await store.upsert_entity(
        ...
        external_id=f"{resource_key}:sec", properties={"data_type": "private_key"},  # a SECRET type
    )
    # the exposed resource node already exists (record_exposed_resources); attach EXPOSES_DATA
    for r in await store.list_entities_by_type(tenant_id=_T, entity_type=NodeCategory.CLOUD_RESOURCE.value):
        if r.external_id == resource_key:
            await store.add_relationship(
                tenant_id=_T, src_entity_id=r.entity_id, dst_entity_id=d,
...
async def test_multicloud_public_resource_is_a_source(resource_key) -> None:
    async with in_memory_semantic_store() as store:
        ...
        node = next(r for r in await store.list_entities_by_type(
            tenant_id=_T, entity_type=NodeCategory.CLOUD_RESOURCE.value) if r.external_id == resource_key)
