...
def _contract(tmp_path) -> ExecutionContract:
    return ExecutionContract(
        ...
        target_agent="identity", customer_id="cust_test", task="fixture run",
...
async def test_fixture_listing_drives_run_and_populates_the_graph(tmp_path) -> None:
    listing = IdentityListing(
        users=(IamUser(arn=_ADMIN, name="admin", user_id="AIDA1", create_date=_DATE,
                       last_used_at=None, attached_policy_arns=(_ADMIN_POLICY,)),),
        roles=(), groups=(),
    )
    async with in_memory_semantic_store() as store:
        # a resource for the admin to reach (data-security/cloud-posture own this in a real run)
        await store.upsert_entity(tenant_id="cust_test", entity_type=NodeCategory.CLOUD_RESOURCE.value,
                                  external_id=_BUCKET, properties={})
        ...
        idents = {r.external_id for r in await store.list_entities_by_type(
        ...
        admin_id = next(r.entity_id for r in await store.list_entities_by_type(
            tenant_id="cust_test", entity_type=NodeCategory.IDENTITY.value) if r.external_id == _ADMIN)
        access = await store.get_relationships_from(
            tenant_id="cust_test", src_entity_id=admin_id, edge_types=(EdgeType.HAS_ACCESS_TO.value,))
        assert access, "run() must write the admin's HAS_ACCESS_TO edges (the dormancy fix in action)"
