...
async def test_coverage_baseline() -> None:
    ...
    async with in_memory_semantic_store() as store:
        ...
        await _expose(store, "arn:aws:s3:::pe")
        ...
        await _expose(store, "arn:aws:s3:::lk")
        ...
        await CloudKgWriter(store, _T).record_stored_secrets(
            stored_secret_grants([("svc:web", [_SK])])
        )
        await ident.record_credential_ownership([("u:bot", _SK)])
        await ident.record_access([("u:bot", "arn:aws:s3:::st")])
        await _expose(store, "arn:aws:s3:::st")
        role = IamRole(
            arn="arn:aws:iam::111111111111:role/p",
            name="p",
            role_id="A",
            create_date=None,  # type: ignore[arg-type]
            last_used_at=None,
        ...
        await _expose(store, "arn:aws:s3:::xa")
        ...
        pv = await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="arn:prv",
            properties={},
        )
        cv = await store.upsert_entity(
            tenant_id=_T,
        ...
        await _expose(store, "arn:aws:s3:::k8")
