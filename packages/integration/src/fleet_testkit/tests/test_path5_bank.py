"""Path-5 (crown jewel) capability bank — all four feeders. TRIVY-GATED."""

from pathlib import Path

import pytest

from fleet_testkit.bank_runner import run_crown_jewel_case
...
_BANK = Path(__file__).parent / "banks" / "path5_crown_jewel"
...
async def test_case_meets_pass_criteria(case: Path) -> None:
    result = await run_crown_jewel_case(case)
    evaluate(result, load_test_case(case).pass_criteria)


@pytest.mark.asyncio
async def test_bank_aggregate_precision_recall() -> None:
    assert _CASES, "no path-5 bank cases found"
    tp = fp = fn = 0
    rows = []
    for case in _CASES:
        r = await run_crown_jewel_case(case)
        tp += r.true_positives
    ...
    print("\n=== Path 5 (crown jewel) capability bank ===")
