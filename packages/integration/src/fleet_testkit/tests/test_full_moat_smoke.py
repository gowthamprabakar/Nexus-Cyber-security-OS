...
async def _vuln_host(store, arn):
    h = await store.upsert_entity(
        tenant_id=_T, entity_type=NodeCategory.CLOUD_RESOURCE.value, external_id=arn, properties={}
    )
    cve = await store.upsert_entity(
        tenant_id=_T,
        entity_type=NodeCategory.CVE_FINDING.value,
        external_id=f"CVE-{arn[-4:]}",
...
async def test_full_moat_report_card() -> None:
    async with in_memory_semantic_store() as store:
        ...
        insts = (
            NetworkInstance("arn:pub-ec2", ("sg-web",)),
            NetworkInstance("arn:priv-ec2", ("sg-db",)),
        )
        sgs = (SecurityGroup("sg-db", (IngressRule("tcp", 5432, 5432, ("sg-web",)),)),)
        await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="arn:pub-ec2",
            properties={"is_public": True},
        )
        ...
        role = IamRole(
            arn="arn:aws:iam::111111111111:role/partner",
            name="partner",
            role_id="AROA1",
            create_date=None,
        ...
        await ident.record_access(
            [("arn:aws:iam::111111111111:role/partner", "arn:aws:s3:::partner")]
        )
        ...
        await k8s.record_inventory(
            ClusterInventory(
                ...
                service_accounts=(
                    K8sServiceAccount(
                        name="ci", namespace="prod", role_arn="arn:aws:iam::1:role/eks"
                    ),
                ),
            )
        )
        await k8s.record_privileged_workloads(
            cl, [PrivilegedWorkload("prod", "foothold", "img:1", "ci")]
        )
        await ident.record_access([("arn:aws:iam::1:role/eks", "arn:aws:s3:::k8s")])
        await _expose(store, "arn:aws:s3:::k8s")
        pods = (
            PodRef(f"{cl}/namespace/prod/pod/foothold", "prod"),
            PodRef(f"{cl}/namespace/prod/pod/victim", "prod"),
        )
        ...
        victim = await store.upsert_entity(
            ...
            external_id=f"{cl}/namespace/prod/pod/victim",
            properties={"kind": "pod"},
        )
        vimg = await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CLOUD_RESOURCE.value,
            external_id="victim-img:1",
            properties={"kind": "container-image"},
        )
        vcve = await store.upsert_entity(
            tenant_id=_T,
            entity_type=NodeCategory.CVE_FINDING.value,
            external_id="CVE-POD-1",
