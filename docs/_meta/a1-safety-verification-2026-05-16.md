...
- ✅ G3: `NEXUS_LIVE_K8S=1` lane ran green at HEAD `96bd75c` — §8 entry 1
- ✅ G4: This record exists

**All four gates closed.** Per the four-gate plan in the post-A.1 readiness report (Tier 0), the cure-quadrant queue is now unblocked. The user's stated order: write the Phase-1c promotion-tracking plan next (§3 gap), because it must land before any v0.2 / further do-agent work.

**The bright line at §5 has shifted but not dissolved.** With G3 green:

- The platform-capability claim is now proven against a live cluster (Stages 1-3 are no longer hypotheses).
- Customer enablement of Stage 3 in any specific environment is still conditional on items 5, 6, 7, 9 of §6 (customer-side prerequisites).
- Customer enablement of Stage 4 is still conditional on items 1-10, where item 2 (promotion tracking in code) is the next platform task.

— recorded 2026-05-16 (post-A.1, safety-verification record; G3 closed entry 1 appended)
