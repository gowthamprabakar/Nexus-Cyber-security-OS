...
| 15   | ✅     | 094dfc7   | _this PR_ | CLI extension (`approve-skill` / `reject-skill` / `list-skills`) + `skill_candidate_store.py` sidecar layer + eval-runner skill-lifecycle extension + 5 new eval cases (11-15). 20 CLI tests + 10 sidecar tests + 5 new cases = 35 net-new tests. |
| 16   | ✅     | _this PR_ | _this PR_ | This verification record + closure.                                                                                                                                                                                                               |

**Test surface at close:** 392 tests across 27 test modules. ruff check + ruff format --check clean.
...
| 011 | PR-flow + branch protection discipline | One-task-one-PR for all 16 tasks. LOW-RISK on 14 tasks; SAFETY-CRITICAL on Tasks 4 + 11. No `--no-verify` / `--no-gpg-sign` shortcuts.                                                               |
...
| **WI-5** Q-ARCH-1 trajectory CLOSES at 3 forbidden subscribers | ✅ GREEN | `_FORBIDDEN_SUBSCRIPTIONS` now contains `remediation` + `supervisor` + `meta_harness`. The trajectory predicted in Supervisor v0.1's WI-5 completes here. No further pending additions in Phase 1.                                          |
...
## Carry-forwards to v0.2.5+ (8 items)

These are **consciously accepted debt** that the v0.2 plan explicitly deferred. Named here so the v0.2.5 plan author and future maintainers can find them.

1. **DSPy+GEPA v0.2.5 follow-up cycle.** The [prompt-optimization strategic analysis](../_meta/prompt-optimization-strategic-analysis-2026-05-23.md) (f1a05a7, #181) lays out DSPy teleprompter integration with GEPA evaluation. v0.2's stub-LLM eval harness + skill-content byte-equal probe are the prerequisite building blocks. v0.2.5 must cite that analysis in its plan and wire the first DSPy optimizer against the Q3 trigger prompt + Stage 7 skill-composition prompt.

2. **Forensic-visibility gap.** v0.2's `audit_emit.py` logs skill-lifecycle events to F.6's audit chain, but the audit payloads don't carry the raw LLM prompt/response. If an auto-deployed skill causes a regression, the audit chain alone can't reproduce the exact LLM call that generated it. The `provenance` frontmatter points to the audit-log path + entry hash, but the LLM transcript itself lives only in the workspace logs. v0.2.5 should add an LLM-transcript audit entry OR extend the candidate sidecar with the raw prompt/response pair.

3. **`--force` bypass absent (by design).** The CLI ships no `--force` flag for `approve-skill` or eval-gate bypass. This was a deliberate v0.2 scope fence — operator safety over operator convenience. v0.2.5 may add `--force` for emergency unblock workflows, but it MUST be paired with a forcible audit entry and an ADR amendment.
...
6. **Audit-chain decision-vs-detail separation.** `skill_triggers.py` reads F.6 audit-chain entries but the `audit_entries` fixture key in eval cases passes pre-filtered entries (action + payload + hash). The production path walks the full F.6 audit log, which carries more detail than the synthetic path. v0.2.5's eval cases should be updated to match the production audit-entry shape if F.6's schema evolves.
...
Task 15 introduced `skill_candidate_store.py` — a sidecar JSON file (`candidate_meta.json`) that sits beside each shadow `SKILL.md`. The SKILL.md YAML frontmatter carries the agentskills.io fields; the sidecar carries the out-of-band fields (`tool_sequence_hash`, `emitted_at`) that the CLI needs to reconstruct a full `SkillCandidate` pydantic object.

This pattern — in-band format (YAML/markdown) + out-of-band sidecar (JSON) — is load-bearing for the approval workflow. The CLI's `approve-skill` / `reject-skill` / `list-skills` commands all read from the sidecar, not from the SKILL.md. v0.2.5's curator (N3) will extend the sidecar with per-skill telemetry fields.
...
The 8 carry-forwards above are the v0.2.5 plan's starting checklist.
