...
| 4. README + runbook v0.3 + this verification record | _(this)_  |     — | README v0.3 banner + dual-source bullet → triple-source bullet + new quick-start 3c (in-cluster). Runbook section 1c.iii adds full Pod-deployment example (ServiceAccount + ClusterRole + ClusterRoleBinding + CronJob manifest). Section 3 dual-invocation example becomes triple-invocation. Verification record (this).                                                                                                                                     |
