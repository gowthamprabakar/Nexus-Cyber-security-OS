...
| **Purpose**             | Macro-to-micro snapshot of platform readiness, refreshed at the close of the **A.1 production-action** ship. **First "do" agent online; cure quadrant opens at 1/3.**                                           |
...
**Verdict.** Today shipped **D.6 v0.3 + A.1 v0.1** end-to-end. The headline is A.1: **the first "do" agent in the platform** opens the cure quadrant at 1/3 and **collapses what was originally three sequential plans** (A.1 recommend-only → A.2 approve-and-execute → A.3 autonomous, projected ~14 weeks across Tier-3 → Tier-2 → Tier-1) into a **single unified agent shipping all three operational tiers** as `--mode` flags gated by safety primitives. **The biggest competitive gap vs Wiz is materially closed in one session.**
...
### Tier 0 — four gates that block every cure-quadrant slice below (do these first)

|   # | Gate                                                                      | Effort    | Status                                                    | Why this blocks                                                                                                                                                                                                                                         |
| --: | ------------------------------------------------------------------------- | --------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
|  G2 | **Lock `--mode execute` OFF by default behind explicit operational flag** | ~30 min   | this session                                              | A customer who runs `--mode execute` without thinking is the failure mode A.1's safety contract exists to prevent. `recommend` + `dry_run` ship; `execute` requires two layers of opt-in (operational flag + `auth.yaml`).                              |
|  G3 | **`NEXUS_LIVE_K8S=1` integration lane against `kind`**                    | ~1 day    | this session                                              | Every safety claim in the runbook (the rollback timer fires, the inverse patch reverts, the post-validation detector re-detects) is currently asserted by mocked tests. Until those claims survive a real cluster, the safety contract is a hypothesis. |
|  G4 | **A.1 safety verification record + earned-autonomy graduation pipeline**  | ~half day | this session                                              | "Why is collapsing Tier-3 → Tier-2 → Tier-1 into one agent safe?" must answer in writing. Promotion criteria from `recommend` → `dry_run` → human-approved `execute` → unattended `execute` are the product, not an afterthought.                       |

**Only after G1-G4 close do A.1 v0.2 / F.3 v0.2 / any other "do" agent enter the queue.** The safety contract becomes the template every future cure-quadrant agent inherits — it gets proven against reality before anything inherits it.
...
1. **The platform shipped the cure quadrant today.** A.1 is the first "do" agent in the platform; the single biggest competitive gap vs Wiz is materially closed. Wiz weighted coverage moved from **52.2% → 54.0% in one session (+1.8pp)** under the corrected math (see [math correction](wiz-coverage-math-correction-2026-05-16.md)). **This is the largest single-session weighted-coverage delta of any cure-quadrant ship** — though smaller than the D.5 multi-cloud-CSPM ship of 2026-05-13-EOD.
...
System is **on-trajectory for Phase 1 GA by 2026-07 / 2026-08**, ~3-4 months ahead of the M12 calendar. **The cure quadrant is open at 1/3**; A.1 v0.1 ships the safety contract that every future "do" agent will inherit — **once that contract is proven against a real cluster, not just unit tests.** **Wiz-weighted coverage at ~54.0%** under corrected math (was understated by ~1.0pp in the uncorrected report); up **~1.8pp** in one session — the largest single-session cure-quadrant delta, but smaller than the May-13 D.5 ship under correct math.

**Recommended action this week — four gates before any v0.2 work or any further "do"-agent build:**

1. **Math correction recorded** ✅ — [`wiz-coverage-math-correction-2026-05-16.md`](wiz-coverage-math-correction-2026-05-16.md) pins the corrected weight distribution.
2. **Lock `--mode execute` OFF by default** behind an explicit operational flag (separate from `auth.yaml`). `recommend` and `dry_run` modes stay fully available and shippable; the autonomous-apply path is gated. **In progress this session.**
3. **Build the `NEXUS_LIVE_K8S=1` integration lane** against a `kind` cluster. The execute path must actually apply a patch to a real cluster, the rollback timer must actually fire against a real cluster, and the inverse patch must actually revert it. Mocked tests do not satisfy this gate. **In progress this session.**
4. **Write the A.1 safety verification record** — define the earned-autonomy graduation pipeline explicitly. Per-action-class promotion from `recommend` → `dry_run` → human-approved `execute` → unattended `execute`. The human never fully leaves — they move from approving each action to owning policy and the kill switch. **In progress this session.**

**Only after all four gates close** do we choose between A.1 v0.2 (more K8s action classes) and F.3 v0.2 (live AWS CSPM). The safety contract becomes the template every future "do" agent inherits — it gets proven against reality before anything builds on top of it.

— recorded 2026-05-16 (end of A.1 session, math-corrected; four-gate plan logged)
