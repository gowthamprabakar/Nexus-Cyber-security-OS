...
| 6    | ✅ done | `4aca3e8` | `read_gcp_iam_findings` — async Cloud Asset Inventory IAM analyser. Flags public exposure (allUsers/allAuthenticatedUsers + impersonation = CRITICAL; + any role = HIGH), owner-role on external users (CRITICAL via `customer_domain_allowlist`), owner-role on user/group/sa (HIGH), editor-role on user (MEDIUM). Stale-SA check deferred to Phase 1c. 24 tests.                         |
| 7    | ✅ done | `4aca3e8` | `normalize_azure` — pure function lifting `AzureDefenderFinding` + `AzureActivityRecord` into F.3 OCSF 2003. Defender severity 1:1 map; Activity Critical/Error→HIGH, Warning→MEDIUM, Informational/Verbose→INFO. Healthy-assessment filter; activity-class filter (iam/network/storage/keyvault only). Per-(sub, source) sequence numbering. 23 tests. Repo-wide 1703 passed / 11 skipped. |
| 8    | ✅ done | `9875806` | `normalize_gcp` — pure function lifting `GcpSccFinding` + `GcpIamFinding` into F.3 OCSF 2003. SCC severity 1:1 map (`SEVERITY_UNSPECIFIED`→INFO); IAM severity 1:1 (analyser pre-grades). INACTIVE-state filter in normalizer. Resource-type inference for SCC; asset_type passthrough for IAM. Per-(project, source) sequence numbering. 14 tests.                                         |
| 9    | ✅ done | `9875806` | NLAH bundle + 21-LOC shim. ADR-007 v1.2 conformance — D.5 is the 5th agent shipped natively against v1.2 (after D.3 / F.6 / D.7 / D.4). README + tools.md + 2 examples (Azure Defender uplift, GCP overly-permissive IAM with full flagging-rule table); LOC-budget test enforces ≤35. 17 tests. Repo-wide 1734 passed / 11 skipped.                                                        |
| 10   | ✅ done | `acf2c31` | `render_summary` — per-cloud breakdown pinned above per-severity sections (Azure / GCP totals + per-source counts); CRITICAL findings pinned (mirrors F.3+D.3+D.4); per-severity sections show cloud + source + truncated resource ID. 15 tests.                                                                                                                                            |
| 11   | ✅ done | `acf2c31` | Agent driver `run()` — 5-stage pipeline (INGEST → NORMALIZE → SCORE → SUMMARIZE → HANDOFF). 4-feed TaskGroup ingest; normalize_azure + normalize_gcp via F.3 re-export; customer_domain_allowlist plumbed through to GCP IAM reader. 8th agent under ADR-007; `(contract, *, llm_provider, ...)` signature confirmed. 9 tests. Repo-wide 1758 passed / 11 skipped.                          |
| 12   | ✅ done | `fd0ebfc` | 10 representative YAML eval cases: clean_multicloud / azure_defender_high / azure_iam_overpermissive / azure_compute_filtered / gcp_scc_critical / gcp_iam_public_impersonation / gcp_scc_inactive_dropped / mixed_clouds / defender_healthy_dropped / gcp_iam_editor_medium.                                                                                                               |
| 13   | ✅ done | `fd0ebfc` | `MultiCloudPostureEvalRunner` registered via `nexus_eval_runners`. **10/10 acceptance** green via `uv run eval-framework run --runner multi_cloud_posture --cases ... --output ...`. 17 tests. Repo-wide 1775 passed / 11 skipped.                                                                                                                                                          |
| 14   | ✅ done | _(this)_  | CLI (`multi-cloud-posture eval` / `multi-cloud-posture run`) — two subcommands; four optional feed flags + repeatable `--customer-domain`. One-line digest; warning on no-feed. 10 tests.                                                                                                                                                                                                   |
| 15   | ✅ done | _(this)_  | README rewritten for v0.1 ship; operator runbook (`runbooks/multicloud_scan.md`, 8 sections). ADR-007 v1.1 + v1.2 conformance verified; v1.3 + v1.4 opt-outs confirmed.                                                                                                                                                                                                                     |
| 16   | ✅ done | _(this)_  | Verification record (`docs/_meta/d5-verification-2026-05-13.md`). Repo-wide 1785 passed / 11 skipped. Coverage 94% on `multi_cloud_posture.*`. **D.5 done; Phase 1b detection track 75% done.**                                                                                                                                                                                             |
