...
| ---- | ---------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | ⬜ pending | —      | Bootstrap — pyproject (BSL 1.1; deps on charter / shared / eval-framework / cloud-posture for the schema re-export). `multi-cloud-posture` CLI + `multi_cloud_posture` eval-runner entry-points declared. Smoke gate. |
| 2    | ⬜ pending | —      | `CloudProvider` enum (AZURE / GCP) + `CSPMFindingType` enum (4 discriminators); re-export F.3's `build_finding` / `Severity` / `AffectedResource`. Composite schema validation tests.                                 |
| 3    | ⬜ pending | —      | `read_azure_findings` tool — Azure Defender for Cloud JSON file parser; async; preserves resource_id / subscription_id / assessment_id under `unmapped`.                                                              |
| 4    | ⬜ pending | —      | `read_azure_activity` tool — Azure Activity Log JSON parser; async; operations classified by `operationName` regex against IAM/network/storage prefixes.                                                              |
| 5    | ⬜ pending | —      | `read_gcp_findings` tool — GCP SCC findings JSON parser; async; preserves resource_name / parent / category under `unmapped`.                                                                                         |
