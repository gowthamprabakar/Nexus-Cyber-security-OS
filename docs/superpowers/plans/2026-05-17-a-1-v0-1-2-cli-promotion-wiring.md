...
| #   | Status     | Commit | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --- | ---------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | ⬜ pending | —      | Wire `--promotion <path>` into `run_cmd`. Implementation: new Click option (Q1), tracker load + `agent_run(promotion=tracker)` plumbing (Q2), no new CLI output paths (Q3), no `--operator` flag (Q4). 3 new CLI tests: (a) flag plumbs tracker → Stage-1 + execute produces `refused_promotion_gate` outcome; (b) absent flag preserves v0.1.1 behaviour; (c) invalid path errors via `click.UsageError`. Update runbook §14 Step 8 + Step 10 to drop "v0.1.2 task" caveats; update README earned-autonomy paragraph similarly. **Hash-pin this row after the task lands.** |
