...
| ---- | ---------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | ⬜ pending | —      | Bootstrap (`pyproject` + BSL 1.1 + `py.typed` + README stub + smoke gate inc. v1.2 NLAH-loader import gate). Mirrors D.3 Task 1.                                                                                                                                               |
| 2    | ⬜ pending | —      | `AuditEventModel` SQLAlchemy table in `charter.memory.models` (shared declarative base; F.5's alembic head extends to `audit_events`). Indexed for (tenant_id, emitted_at DESC), (tenant_id, action), (correlation_id).                                                        |
| 3    | ⬜ pending | —      | Alembic `0003_audit_events` migration drives the Task 2 model. RLS policy lives in this migration (Postgres-gated; aiosqlite no-op).                                                                                                                                           |
| 4    | ⬜ pending | —      | OCSF v1.3 Audit Activity schema (`class_uid 2007`) + pydantic models — `AuditEvent`, `AuditQueryResult`, `ChainIntegrityReport`. Frozen, validated, JSON-round-tripping.                                                                                                       |
| 5    | ⬜ pending | —      | `AuditStore` typed async accessor over `audit_events`: `ingest(events)`, `query(tenant_id, *, since, until, action, agent_id, correlation_id, limit)`, `count_by_action(tenant_id, since, until)`. RLS-aware.                                                                  |
| 6    | ⬜ pending | —      | File-based ingest tool — `audit_jsonl_read(path)` async wrapper following the ADR-007 v0.5 async-tool convention. Reads `audit.jsonl` files emitted by `charter.audit.AuditLog`.                                                                                               |
| 7    | ⬜ pending | —      | Memory-table ingest tool — `episode_audit_read(memory, *, tenant_id, since, until)` — reads from `episodes` table (the F.5 memory engine's audit emissions) and converts to `AuditEvent` shape.                                                                                |
| 8    | ⬜ pending | —      | Chain-integrity verifier — `verify_audit_chain(events, *, sequential)` wraps `charter.verifier.verify_audit_log` for in-memory event lists; reports `ChainIntegrityReport` with `broken_at_correlation_id`.                                                                    |
| 9    | ⬜ pending | —      | Markdown summarizer (top-down severity: tamper-detected → high-volume action → per-tenant breakdown) and `chain_breaks_pinned` section above per-action sections (mirrors D.3's "Critical runtime alerts pin").                                                                |
