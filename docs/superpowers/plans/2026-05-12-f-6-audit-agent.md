...
| 10   | ✅ done | `e7837ff`     | NLAH bundle + 27-LOC shim (well inside the ≤35 LOC v1.2 ceiling). README + tools.md + 2 examples (compliance-export + tamper-detected). F.6 is the **second** agent shipped natively on v1.2 (after D.3). 8 tests. Repo-wide 1124 passed / 11 skipped.                                                                                                                                                                                        |
| 11   | ✅ done | `ab28b13`     | `translate_nl_query(nl, tenant_id, provider)` via `charter.llm` Protocol. Three safety properties: tenant pivot impossible (stamped, not LLM-supplied); malformed LLM output non-fatal (falls back to tenant-only); LLM unavailable non-fatal. Per-field drop on validation failure. 9 tests. Repo-wide 1133 passed / 11 skipped.                                                                                                             |
| 12   | ✅ done | `5ddcf60`     | `audit.agent.run` — TaskGroup fan-out across jsonl sources + memory; chain verification (sequential for jsonl, non-sequential for memory); AuditStore.ingest + query; writes report.md + events.json. **ADR-007 v1.3 always-on policy locked in `_enforce_always_on`**: only `wall_clock_sec` raises; every other axis logs a warning + proceeds. 9 tests (incl. budget-axis warning + wall-clock raise). Repo-wide 1142 passed / 11 skipped. |
| 13   | ✅ done | `df9c413`     | 10 YAML cases covering every flow (empty / clean chain / tampered / per-action / tenant-isolation / cross-source merge / time-range / agent_id / correlation-walk / NL-translation). Fixture schema: jsonl_events + memory_events + tampered_jsonl_index + query{} + nl_query + llm_response.                                                                                                                                                 |
| 14   | ✅ done | `42346c7`     | `AuditEvalRunner` registered via `[project.entry-points."nexus_eval_runners"] audit`. Materializes fixtures into hash-chained jsonl + F.5 episodes seed; stubs `LLMProvider` for case 010's NL path. **All 10/10 cases pass.** 15 tests. Repo-wide 1157 passed / 11 skipped.                                                                                                                                                                  |
| 15   | ✅ done | `f07f2b9`     | `audit-agent eval / run / query`. Exit codes: 0=clean, 1=tooling, 2=chain tamper (lets cron jobs distinguish without parsing stderr). `query` supports markdown / json / csv outputs + all five filter axes. 11 tests. Repo-wide 1168 passed / 11 skipped.                                                                                                                                                                                    |
| 16   | ✅ done | _this commit_ | Full operator README + `runbooks/audit_query_operator.md` (8-section walkthrough). **ADR-007 v1.3 amendment ratified** ("always-on agent class"; F.6 is the first member). Final verification record at `docs/_meta/f6-verification-2026-05-12.md`. Coverage 96% (`audit.*`); all gates green. **Phase 1a foundations closed.**                                                                                                               |
