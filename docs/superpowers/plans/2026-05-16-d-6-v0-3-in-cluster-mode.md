...
| ---- | ---------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | ⬜ pending | —      | `read_cluster_workloads` gains `in_cluster: bool = False` parameter. `_read_sync` branches on it (calls `config.load_incluster_config()` instead of `load_kube_config`). Q2 mutual exclusion enforced (kubeconfig XOR in_cluster). v0.3 smoke test asserts the `load_incluster_config` symbol is available. Reader unit tests for: success path, ConfigException → ClusterReaderError, mutual-exclusion error. |
| 2    | ⬜ pending | —      | `agent.run` gains `in_cluster: bool = False` param. `_ingest` routes through `read_cluster_workloads(in_cluster=True, ...)` when set. Q2 mutual exclusion at the agent-level too (matches v0.2 pattern). Unit tests for the routing.                                                                                                                                                                           |
| 3    | ⬜ pending | —      | CLI `--in-cluster` boolean flag. Mutually-exclusive checks vs `--kubeconfig` + `--manifest-dir`; `--cluster-namespace` works with either `--kubeconfig` OR `--in-cluster`. CLI unit tests for the new flag + the new error cases.                                                                                                                                                                              |
| 4    | ⬜ pending | —      | README v0.3 banner; runbook section 1c.iii (in-cluster ServiceAccount example with full Pod manifest + SA + ClusterRoleBinding); v0.3 verification record `docs/_meta/d6-v0-3-verification-<date>.md`.                                                                                                                                                                                                         |
