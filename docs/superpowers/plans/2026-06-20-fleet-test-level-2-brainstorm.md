...
| compliance          | 2003          | (framework-control id, source-finding id set)             | an attestation correctly names the failing control + the findings supporting it. TP = right control AND right cited findings; FN = a should-fail control not attested failing; FP = cites findings that don't exist / don't support the claim (compliance theater) |
