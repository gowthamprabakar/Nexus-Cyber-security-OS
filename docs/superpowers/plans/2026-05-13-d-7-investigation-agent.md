...
| 7    | ✅ done    | `d201ef9` | `map_to_mitre(evidence)` heuristic mapper. Bundled ATT&CK 14.1 JSON (10 techniques covering shipped-agent evidence shapes). Ranks by keyword hits desc, technique_id asc. No T0000 fallback — empty tuple is itself a signal. 14 tests. Repo-wide 1267 passed / 11 skipped.                                                                           |
| 8    | ✅ done    | `114d9d2` | **Sub-agent orchestrator (Q2 resolution).** `SubAgentOrchestrator.spawn_batch` — allowlist-enforced (one entry: `investigation`), depth cap 3, parallel cap 5, TaskGroup-based concurrency. v0.1 runs sub-investigations as scoped workers under parent Charter (per-sub-Charter deferred to Phase 1c). 13 tests. Repo-wide 1280 passed / 11 skipped. |
| 9    | ✅ done    | `bb84e62` | `reconstruct_timeline` pure-function merger. Three input sources (AuditEvent, RelatedFinding, TimelineEvent) → sorted `Timeline`. Forgiving on missing timestamps or finding_info.uid. 10 tests. Repo-wide 1290 passed / 11 skipped.                                                                                                                  |
| 10   | ✅ done    | `2806630` | NLAH bundle + 21-LOC shim. ADR-007 v1.2 conformance (3rd agent natively on v1.2 after D.3 + F.6). README + tools.md + 2 examples (shell-in-container happy-path + LLM-unavailable fallback). 8 tests. Repo-wide 1298 passed / 11 skipped.                                                                                                             |
| 11   | ✅ done    | `9a7b997` | `synthesize_hypotheses` — **load-bearing LLM use** (first such agent). Mandatory evidence-validation drops hallucinated hypotheses; partial hallucinations drop in full. LLM-unavailable → deterministic fallback (one hypothesis per finding, confidence 0.5). 11 tests. Repo-wide 1309 passed / 11 skipped.                                         |
| 12   | ✅ done    | `af5edf8` | Agent driver `run()` — 6-stage pipeline (SCOPE→SPAWN→SYNTHESIZE→VALIDATE→PLAN→HANDOFF). Writes 4 artifacts. SPAWN uses `SubAgentOrchestrator` to fan out 4 sub-investigations in parallel. PLAN emits per-class_uid containment templates. 9 tests. Repo-wide 1318 passed / 11 skipped.                                                               |
...
| 16   | ✅ done    | _(this)_  | README + operator runbook (`runbooks/investigation_workflow.md`, 7 sections). ADR-007 v1.4 evaluated + **deferred** (one consumer; rehoist when third duplicate appears). Verification record: `docs/_meta/d7-verification-2026-05-13.md`. Repo-wide 1340 passed / 11 skipped. Coverage 94% on `investigation.*`. **D.7 done; Phase 1b open.**        |
...
| --- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| Q1  | Which OCSF class_uid for incident findings?                                 | **Plan-corrected at Task 2.** Initial choice was `2004 Detection Finding + types[0]="incident"`; close reading of OCSF v1.3 confirmed `2005 Incident Finding` is the canonical purpose-built class. Mirrors F.6's `2007→6003` correction.                                                               | Task 2     |
