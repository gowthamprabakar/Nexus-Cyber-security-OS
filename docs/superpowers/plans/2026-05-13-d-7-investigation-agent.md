...
| ---- | ---------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | ⬜ pending | —      | Bootstrap (`pyproject` + BSL 1.1 + `py.typed` + README stub + smoke gate inc. F.5 SemanticStore + F.6 AuditStore import checks)                                                                                                                                        |
| 2    | ⬜ pending | —      | OCSF schemas (`IncidentFinding`, `Timeline`, `TimelineEvent`, `Hypothesis`, `IocItem`, `MitreTechnique`, `IncidentReport`). **Q1: class_uid verification.** Frozen, validated, JSON-round-tripping.                                                                    |
| 3    | ⬜ pending | —      | `audit_trail_query(*, audit_store, tenant_id, since, until, ...)` async tool — wraps F.6 `AuditStore.query`. Mirrors the ADR-005 async-tool convention.                                                                                                                |
| 4    | ⬜ pending | —      | `memory_neighbors_walk(*, semantic_store, tenant_id, entity_id, depth, edge_types)` async tool — wraps F.5 `SemanticStore.neighbors`. Respects depth ≤ `MAX_TRAVERSAL_DEPTH`.                                                                                          |
| 5    | ⬜ pending | —      | `find_related_findings(sibling_workspaces)` async tool — reads `findings.json` from each path, deserialises into the shared OCSF Finding shape, returns the merged tuple.                                                                                              |
| 6    | ⬜ pending | —      | `extract_iocs(text \| dict)` — regex + heuristic IOC extraction. Output is a `tuple[IocItem, ...]`. IOC types: IPv4 / IPv6 / domain / URL / SHA-256 / SHA-1 / MD5 / email / CVE. Pure function (no async needed); still wrapped per ADR-005 for budget tracking.       |
