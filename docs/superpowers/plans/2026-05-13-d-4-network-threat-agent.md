...
| 5    | ✅ done | `34312bb` | `read_dns_logs` — multi-format dispatch (first-line peek picks BIND vs. Route 53 JSON); BIND regex with `%f` 1-6 digit subseconds; Route 53 answers / vpc_id / unmapped; trailing-dot + lowercase qname normalisation. 14 tests.                                                                                                                                                                                              |
| 6    | ✅ done | `34312bb` | `detect_port_scan` — sliding-window heuristic; defaults 50 ports / 60s; severity escalation (>=200 CRITICAL / >=100 HIGH / >=50 MEDIUM); loopback / link-local / unspecified suppression; ACCEPT-only; per-src skip-ahead dedupe. 21 tests. Repo-wide 1449 passed / 11 skipped.                                                                                                                                               |
| 7    | ✅ done | `ee4fb54` | `detect_beacon` — periodicity analysis grouped by (src,dst,port); defaults min_count=5 / max_cov=0.30 / min_period=1.0s per Q3; severity escalates on count+CoV combos; confidence in [0,1]; loopback/link-local filtered. 22 tests.                                                                                                                                                                                          |
| 8    | ✅ done | `ee4fb54` | `detect_dga` — Shannon entropy + Norvig top-50 bigram heuristic on the second-level label per Q2 (no ML in v0.1); defaults entropy≥3.5, bigram≤0.30, length≥7; severity HIGH at entropy≥4.0 + bigram≤0.05; bundled suffix allowlist (CloudFront/S3/Google/Azure/Fastly/Cloudflare); dedup by (src,qname). 25 tests. Repo-wide 1496 passed / 11 skipped.                                                                       |
| 9    | ✅ done | `7acf6a5` | `enrich_with_intel` — bundled static intel snapshot (CISA KEV + abuse.ch + MITRE refs); 16 known-bad domains + 12 known-bad IP CIDRs + 10 Tor exit CIDRs. Pure function over Detection sequence; annotates `evidence.intel` with tags + matched_cidr / matched_domain_suffix. Severity uplift on match (capped at CRITICAL); SURICATA never enriched. Longest-suffix-match preferred. 17 tests.                               |
| 10   | ✅ done | `7acf6a5` | NLAH bundle + 21-LOC shim. ADR-007 v1.2 conformance — D.4 is the 4th agent shipped natively against v1.2 (after D.3 / F.6 / D.7). README + tools.md + 2 examples (beacon + DGA-domain); LOC-budget test enforces ≤35. 10 tests. Repo-wide 1523 passed / 11 skipped.                                                                                                                                                           |
| 11   | ✅ done | `75e0906` | `render_summary` — pinned beacons + DGA above per-severity sections; severity + finding-type breakdowns; empty-report fast path. Pin order: beacons → DGA → per-severity (Critical→Info). 15 tests.                                                                                                                                                                                                                           |
| 12   | ✅ done | `75e0906` | Agent driver `run()` — 6-stage pipeline. INGEST 3-feed TaskGroup; PATTERN_DETECT runs the 3 pure detectors + Suricata lift; ENRICH annotates + uplifts; SCORE/dedup via `Detection.dedup_key()` (Q6); SUMMARIZE/HANDOFF writes findings.json + report.md + audit chain. ADR-007 sig converges (7th agent). 27 tests. Repo-wide 1546 passed / 11 skipped.                                                                      |
| 13   | ✅ done | `7050a1b` | 10 representative YAML eval cases: clean_network / port_scan_at_threshold / beacon_low_variance / beacon_high_variance / dga_high_entropy / dga_low_entropy_skipped / suricata_alert_only / intel_uplift_tor_exit / three_feed_merge / allowlisted_cloudfront_suppressed. Synthesizer directives `flow_records_scan` + `flow_records_beacon` keep YAML compact.                                                               |
| 14   | ✅ done | `7050a1b` | `NetworkThreatEvalRunner` registered via `nexus_eval_runners`. **10/10 acceptance** green via `uv run eval-framework run --runner network_threat --cases ... --output ...`. 16 tests. Repo-wide 1562 passed / 11 skipped.                                                                                                                                                                                                     |
| 15   | ✅ done | `1930dcb` | CLI (`network-threat eval` / `network-threat run`) — two subcommands per ADR-007 shape; three optional feed flags (`--suricata-feed` / `--vpc-flow-feed` / `--dns-feed`). One-line digest with severity + finding-type breakdown; warning on no-feed. 9 tests. (Filter axes `--since`/`--until`/`--src-cidr` deferred — feed-pinning via the contract envelope is the v0.1 shape.)                                            |
| 16   | ✅ done | `1930dcb` | README rewritten for v0.1 ship; operator runbook (`runbooks/network_triage.md`, 8 sections). ADR-007 v1.1 + v1.2 conformance verified; v1.3 opt-out; v1.4 not consumed (no amendments surfaced). Verification record: `docs/_meta/d4-verification-2026-05-13.md`. Repo-wide 1571 passed / 11 skipped. Coverage 94% on `network_threat.*`. **D.4 done; Phase 1b half-done.**                                                   |
