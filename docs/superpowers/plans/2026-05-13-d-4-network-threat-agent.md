...
| ---- | ---------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | ⬜ pending | —      | Bootstrap — pyproject (BSL 1.1, depends on `nexus-audit-agent` for cross-agent audit consumer), entry-point, README stub. Smoke gate covers ADR-007 v1.1/v1.2.                                         |
| 2    | ⬜ pending | —      | OCSF schemas — Detection Finding 2004 with `types[0]="network_threat"`. 6 pydantic models (Finding / FlowRecord / DnsEvent / SuricataAlert / Beacon / Detection).                                      |
| 3    | ⬜ pending | —      | `read_suricata_alerts` tool — ndjson parser; async-via-`asyncio.to_thread`; forgiving on malformed lines; mirrors `audit_jsonl_read`.                                                                  |
| 4    | ⬜ pending | —      | `read_vpc_flow_logs` tool — AWS Flow Logs v5 format (space-separated 26-field records); async; gzipped + plaintext support.                                                                            |
