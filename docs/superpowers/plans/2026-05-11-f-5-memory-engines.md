...
| 2    | ✅ done | `0f01599`     | Alembic config + `0001_memory_baseline` migration that drives the Task 1 models; tests execute the migration end-to-end (offline-mode SQL emit + online `upgrade head` against aiosqlite); `version_table = "alembic_version_memory"` so it coexists with control-plane's alembic head when sharing a Postgres. 11 tests; mypy strict clean (23 source files); repo-wide 946 passed / 5 skipped.                                                   |
| 3    | ✅ done | `f7c0eb3`     | `EpisodicStore` — typed async accessor; `append_event(*, tenant_id, correlation_id, agent_id, action, payload, embedding=None)` + `query_by_correlation_id` + `query_recent` + `search_similar` (Postgres pgvector ANN; aiosqlite degrades to []). 9 tests; mypy strict clean (101 source files); repo-wide 955 passed / 5 skipped.                                                                                                                |
| 4    | ✅ done | `723cbc7`     | `Embedding` Protocol + `FakeEmbeddingProvider` — SHA-256-derived, deterministic, L2-unit-normalised vectors at configurable dim (default 1536). Production providers (OpenAI / Anthropic) drop in via the same Protocol seam in Phase 1b. Resolves Q2 (eager-embed-on-write). 16 tests; mypy strict clean (102 source files); repo-wide 971 passed / 5 skipped.                                                                                    |
| 5    | ✅ done | `b98679e`     | `ProceduralStore` — versioned playbook CRUD; `publish_version` flips prior active row + inserts new in one txn (exactly-one-active invariant); `get_active` / `list_versions` / `list_subtree` (Postgres LTREE `<@`; aiosqlite `LIKE` fallback). 12 tests; mypy strict clean (103 source files); repo-wide 983 passed / 5 skipped.                                                                                                                 |
| 6    | ✅ done | `13bac64`     | `SemanticStore` — entity/relationship CRUD + BFS traversal; `upsert_entity` idempotent on natural key with property-merge semantics; `add_relationship` directed edges; `neighbors(depth ≤ 3, edge_types=...)` for graph walks. Adds `python-ulid` to charter deps. 13 tests; mypy strict clean (104 source files); repo-wide 996 passed / 5 skipped.                                                                                              |
| 7    | ✅ done | `5d728c9`     | Per-tenant RLS migration `0002_memory_rls` — `ENABLE ROW LEVEL SECURITY` + `tenant_isolation` policy on every memory table (Postgres-gated; aiosqlite no-op). Resolves Q3. 9 structural tests; live "off-tenant returns empty" assertion lands in Task 10. Mypy strict clean (104 source files); repo-wide 1005 passed / 5 skipped.                                                                                                                |
| 8    | ✅ done | `2094dec`     | Charter audit-chain instrumentation on every store write. Action constants locked at module scope (`episode_appended` / `playbook_published` / `entity_upserted` / `relationship_added`). All three stores take optional `audit_log=` kwarg; emits happen after commit (no phantom entries on rollback). 7 tests inc. mixed-engine chain verifies via `verify_audit_log`. Mypy strict clean (105 source files); repo-wide 1012 passed / 5 skipped. |
| 9    | ✅ done | `a990b4c`     | `MemoryService` facade — single DI seam; bundles the three stores + embedder + audit_log. `session(tenant_id)` is an async ctx mgr that does `SET LOCAL app.tenant_id` on Postgres for RLS. `append_event(...)` runs the embedder by default; caller-supplied embedding overrides. 8 tests inc. cross-tenant isolation + audit threading + embedder wiring. Mypy strict clean (106 source files); repo-wide 1020 passed / 5 skipped.               |
| 10   | ✅ done | `f497c91`     | Live Postgres integration test — opt-in via `NEXUS_LIVE_POSTGRES=1`; docker-compose `postgres` swapped to `pgvector/pgvector:pg16`. 6 tests: alembic+extensions, CRUD on all 4 tables, pgvector top-K ranking against 100 deterministic embeds, RLS isolation via raw SQL bypassing app-side filter. Mypy strict clean (106 source files); repo-wide 1020 passed / 11 skipped.                                                                     |
| 11   | ✅ done | `32f2129`     | Operator runbook `packages/charter/runbooks/memory_bootstrap.md` — 8 sections covering dev-compose / Cloud SQL / RDS bring-up, alembic upgrade, psql smoke, `MemoryService` wiring snippet, prod-hardening checklist (PgBouncer txn-mode, BYPASSRLS gotcha, ivfflat re-tune), troubleshooting table.                                                                                                                                               |
| 12   | ✅ done | _this commit_ | ADR-009 (memory architecture) drafted: 6 load-bearing decisions (Postgres collapse, dialect-portable types, distinct alembic head, RLS primary defence, Embedding Protocol, MemoryService facade). F.5 verification record at `docs/_meta/f5-verification-2026-05-12.md`. Coverage 95% (`charter.memory`); all gates green.                                                                                                                        |
