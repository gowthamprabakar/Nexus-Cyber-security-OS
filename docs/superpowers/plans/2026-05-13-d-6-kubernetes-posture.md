...
| 14   | ✅ done | `7717210` | `K8sPostureEvalRunner` + `nexus_eval_runners` entry-point. **10/10 acceptance gate** via parametrized test against the shipped YAML suite. Fixture parsers map dict → KubeBenchFinding / PolarisFinding / ManifestFinding. 17 tests (4 protocol/synthetic + 1 dedup-passthrough + 10 case acceptances + 1 entry-point + 1 case-count).     |
| 15   | ✅ done | `7717210` | CLI `k8s-posture eval CASES_DIR` (prints `N/M passed`; non-zero exit on any failure) and `k8s-posture run --contract …` with three optional feed flags (`--kube-bench-feed` / `--polaris-feed` / `--manifest-dir`). 10 tests including 10/10-passes-via-CLI smoke. Repo-wide 2030 passed / 11 skipped.                                     |
| 16   | ✅ done | _(this)_  | README rewritten (full operator-facing content); operator runbook (`runbooks/k8s_scan.md`, 9 sections); verification record (`docs/_meta/d6-verification-2026-05-13.md`); 245 tests; 97% coverage; **Phase-1b detection track CLOSED 2026-05-13** (D.7 + D.4 + D.5 + D.6 all shipped at M2).                                               |
