...
| 13  | Identity privilege-escalation chain (transitive assume) | ❌       | generic-engine candidate only                   |
...
**Covered: 12 full + 2 partial of 22 = ~55%** (12/22 = 0.545; counting partials as half → ~59%). **This lands the "~50-60% of Wiz" claim — now with an explicit list, not a feel.**

## What the gaps tell us (the breadth backlog, ranked)

Highest-value uncovered (each ~1 named-archetype slice):

1. **#13 privilege-escalation chain** — generic engine already surfaces candidates; promote to named.
...
**Verdict:** detection is **~55% coverage / 1.000 quality on what it covers**. "Complete per North Star" needs **B (live loop)** + a few **C** slices to push coverage toward 60-65%. The number is now real.
